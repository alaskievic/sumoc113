#!/usr/bin/env python3
"""OCR questionario_agri PDFs with Mistral Document AI OCR.

This runner writes outputs in the same broad shape as the existing PaddleOCR
runner: a raw JSON response plus a page-delimited text file that downstream
section parsers can inspect or adapt.
"""
...
def load_workspace_env() -> None:
    ...
    for env_path in (WORKSPACE_ROOT / ".env.local", WORKSPACE_ROOT / ".env"):
...
def multipart_body(fields: dict[str, str], file_field: str, file_path: Path) -> tuple[bytes, str]:
    boundary = f"----quest-agri-mistral-{uuid.uuid4().hex}"
...
def run_ocr(
    *,
    ...
    try:
        ...
        # Some API surfaces document uploaded files without the explicit type.
        # Retry once with the leaner documented FileChunk shape.
        if '"type": "file"' not in body.decode("utf-8"):
            raise
...
def normalized_result(pdf_path: Path, model: str, response: dict[str, Any]) -> dict[str, Any]:
    pages = []
...
def main() -> None:
    ...
    page_indexes = parse_page_list(args.page_list)
    started = time.time()
    ...
    try:
        ...
        response = run_ocr(
            ...
            confidence_scores_granularity=args.confidence_scores,
        )
    finally:
    ...
    result = normalized_result(args.pdf, args.model, response)
    result["elapsed_seconds"] = round(time.time() - started, 3)
    result["remote_file_id"] = file_id if args.keep_remote_file else None

    args.json_out.parent.mkdir(parents=True, exist_ok=True)
    args.json_out.write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
    write_text(result, args.text_out)
    print(
        f"Wrote {len(result['pages'])} pages to {args.json_out} and {args.text_out}",
        flush=True,
    )
